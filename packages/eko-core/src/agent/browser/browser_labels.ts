...
* BROWSER OPERATION:
  ...
  protected async handleMessages(
    ...
    const pseudoHtmlDescription =
      "请你按如下步骤完成输出：" +
        "一、先评估从当前截图中看到的执行结果是否符合预期，用拟人化的语气将看到的结果分点简洁列出，例如当正确执行时使用“太好了! 我看到...”, 或者“完美！我观察到...”, 当执行有误不符合预期，或者没有变化时使用“看起来似乎不太对，我发现...”注意在描述元素时务必要补充元素所在的位置区域信息描述。\n" +
        "二、结合最新的截图和页面元素信息，根据当前的任务进展，分析是否存在非工具类任务，如生成总结、生成文本、生成描述等信息汇总类任务。" +
        "当存在非工具类任务时，必须先详细输出非工具类任务的执行结果后再去执行步骤三操作。注意：1. 思考当前页面的信息是否是完整信息，如果不完整，需要尝试利用滚动工具获取完整信息。2. 不要让人感知到当前在进行非工具类任务。3. 不要输出“非工具类任务”相关字眼。\n" +
        "三、最后用一句话说明接下来要执行的一个操作是什么，例如“接下来我会执行...”, 注意：" +
        "1. 页面中各种元素都可以利用起来，如果没有直接符合描述的元素，可以思考是否可以通过其他方法达到同样的效果。" +
        "2. 如果页面元素信息中有操作相关的信息，但没有编号，同时截图中也没有这个元素信息，可能是不在可视网页范围内，需要滚动直到获取到想要的元素或到达页面边界为止。" +
        "3. 生成的执行操作需要和上文的观察比对，确定区域和描述是否正确。" +
        "4. 优先处理弹窗、浮层。如果全部任务都已经完成，明确表达出任务已经完成的意思。\n" +
